...
class BulkRenewViewTests(TestCase):
    ...
    def _post(self, ids, **kwargs):
        ...
        data = {
            ...
            "importo": "30.00",
            "stato": "pagata",
            "data_inizio": today.replace(month=1, day=1).strftime("%Y-%m-%d"),
            "data_scadenza": today.replace(month=12, day=31).strftime("%Y-%m-%d"),
        }
        data.update(kwargs)
    ...
    def test_get_shows_correct_soci(self):
        response = self._get([self.s1.pk])
        self.assertContains(response, self.s1.codice_fiscale)

    def test_post_creates_quotas(self):
        # Delete auto-created quotas first
        Quota.objects.all().delete()
        self._post([self.s1.pk, self.s2.pk])
        self.assertEqual(Quota.objects.count(), 2)

    def test_post_skips_existing_quota(self):
        # s1 already has auto-created quota, s2 too
        initial_count = Quota.objects.count()
        self._post([self.s1.pk, self.s2.pk])
        self.assertEqual(Quota.objects.count(), initial_count)
    ...
    def test_post_with_stato_in_attesa(self):
        Quota.objects.all().delete()
        self._post([self.s1.pk], stato="in_attesa")
        quota = Quota.objects.get(socio=self.s1)
        self.assertEqual(quota.stato, "in_attesa")
